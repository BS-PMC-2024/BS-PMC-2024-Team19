...
import { Box, Grid, Card, CardContent, Avatar, Typography, Button } from '@mui/material';
              ...
              <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                  <Avatar
                    ...
                    alt={`${stock.name} logo`}
                    onError={(e) => { e.target.src = placeholderImage; }} // Use placeholder on error
                    sx={{ marginRight: 2 }}
                  />
                  <Box sx={{ flexGrow: 1 }}>
                    <Typography variant="h6" sx={{ display: 'flex', justifyContent: 'space-between' }}>
                      {stock.name}
                      <Typography variant="body1" sx={{ fontWeight: 'bold', ml: 2 ,fontSize: '1.5rem'}}>
                        ${stock.currentPrice}
                      </Typography>
                    </Typography>
                    <Typography
